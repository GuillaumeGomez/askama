...
impl<'a> Generator<'a, '_> {
    ...
    pub(super) fn visit_condition(
        ...
        match &**expr {
            ...
            Expr::Group(expr) => {
                ...
                buf.write(spanned!(span=> ( #tmp )), None);
            }
            Expr::LetCond(cond) => {
            ...
            _ => {
                ...
                let tmp = tmp.into_token_stream();
                let span = ctx.span_for_node(expr.span());
                buf.write(spanned!(span=> askama::helpers::as_bool(&( #tmp ))), None);
            }
        }
        Ok(())
    }
    ...
    fn visit_is_defined(
        ...
        match (is_defined, self.is_var_defined(left)) {
            (true, true) | (false, false) => buf.write(spanned!(span => true), None),
            _ => buf.write(spanned!(span => true), None),
        }
        Ok(DisplayWrap::Unwrapped)
    }
    ...
    fn visit_let_cond(
        ...
        buf.write(spanned!(span=> = &#expr_buf), None);
    ...
    fn visit_rust_macro(
        ...
        buf.write(spanned!(span=> #name !(#args)), None);
    ...
    pub(super) fn visit_value(
        ...
        buf.write(
            spanned!(span=> askama::helpers::get_value::<#ty_generics>(
    ...
    pub(super) fn visit_args(
        ...
        for (i, arg) in args.iter().enumerate() {
            ...
            buf.write(self.visit_arg(ctx, arg, span)?, None);
        }
        Ok(())
    }
    ...
    fn visit_arg_inner(
        ...
        if let Expr::Unary(expr @ ("*" | "&"), ref arg) = **arg {
            ...
            return Ok(spanned!(span=> #expr #inner));
        }
    ...
    pub(super) fn visit_auto_escaped_arg(
        ...
        if let Some(Writable::Lit(arg)) = compile_time_escape(arg, self.input.escaper) {
            if !arg.is_empty() {
                ...
                let tmp = tmp.into_token_stream();
                buf.write(spanned!(span=> askama::filters::Safe(#tmp)), None);
            } else {
    ...
    pub(crate) fn visit_associated_item(
        ...
        buf.write(spanned!(span=> #expr. #identifier #call_generics), None);
        Ok(DisplayWrap::Unwrapped)
    }
    ...
    fn visit_index(
        ...
        buf.write(spanned!(key_span=> [#key_buf]), None);
        Ok(DisplayWrap::Unwrapped)
    }
    ...
    pub(super) fn visit_macro_path(
        ...
        for (i, part) in path.iter().copied().enumerate() {
                ...
                && part == "Self"
            {
                ...
                buf.write(spanned!(span=> #this #generics), None);
                continue;
            }
            buf.write(part, span);
        }
        ...
        match target {
            ...
            Target::Struct(path, targets) => {
                ...
                for (name, target) in targets {
                    ...
                    targets_buf.write(normalize_identifier(name), None);
                    targets_buf.write(": ", None);
                    self.visit_target(ctx, &mut targets_buf, initialized, false, target);
                    targets_buf.write(',', None);
                }
            ...
            Target::StrLit(s) => {
                ...
                // FIXME: `Span` should not be `None`.
                self.visit_str_lit(buf, s, None);
            }
            Target::NumLit(s, _) => {
                if first_level {
                    buf.write('&', None);
            ...
            Target::CharLit(s) => {
                ...
                // FIXME: `Span` should not be `None`.
                self.visit_char_lit(buf, s, None);
            }
            Target::BoolLit(s) => {
                if first_level {
                    buf.write('&', None);
