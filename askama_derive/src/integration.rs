...
impl Buffer {
    ...
    fn handle_str_lit(&mut self) {
        ...
        match str_literals.as_slice() {
            ...
            [(literal, span)] => {
                let span = *span;
                self.buf.extend(spanned!(span=> __askama_writer.write_str(#literal)?;));
            }
            [(literal, span), rest @ ..] => {
                let (lit, span) = rest.iter().fold((literal.clone(), *span), |(mut acc_lit, acc_span), (literal, span)| {
                    acc_lit.push_str(literal);
                    (acc_lit, acc_span.join(*span).unwrap_or(acc_span))
                });
                self.buf.extend(spanned!(span=> __askama_writer.write_str(#lit)?;));
            }
        }
    }
